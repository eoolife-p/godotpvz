...
#var garlic_timer:float = 0.0
#var garlic_max:float = 1.35
func ready_norm():
...
func _process(_delta: float) -> void:
	...
	if is_zombie_mode:
		if is_pogo and global_position.x < eat_brain_glo_pos_x_in_zombie_mode:
			loss_iron_item()
	#if not is_big_pogo and garlic_timer >= garlic_max:
		##update_lane_on_eat_garlic() # 触发大蒜效果
		#speed_scale_pogo+=0.4
		#garlic_timer-=garlic_max
