...
func _start_bomb_fx():
	...
	EventBus.push_event("jalapeno_bomb_effect_row", [owner_plant.row_col.y])
...
func _bomb_all_enemy():
	...
	#EventBus.push_event("jalapeno_bomb_lane_zombie_row", [owner_plant.row_col.y])
	## 销毁当前行道具[冰道和梯子]
	EventBus.push_event("jalapeno_bomb_item_lane_row", [owner_plant.row_col.y])
