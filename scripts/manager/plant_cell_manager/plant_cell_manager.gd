...
func _ready() -> void:
	...
	EventBus.subscribe("jalapeno_bomb_effect_row", jalapeno_bomb_effect_row)
	
	## 火爆辣椒销毁道具[冰道和梯子]
	EventBus.subscribe("jalapeno_bomb_item_lane", jalapeno_bomb_item_lane)
	EventBus.subscribe("jalapeno_bomb_item_lane_row", jalapeno_bomb_item_lane_row)
...
## 火爆辣椒爆炸特效
## [lane:int]:列
func jalapeno_bomb_effect_row(lane:int):
	for plant_cell:PlantCell in all_plant_cells[lane]:
		var fire_new:BombEffectFire = SceneRegistry.FIRE.instantiate()
		## 修改其图层
		fire_new.z_index = lane * 50 + 40
		...
		plant_cell.add_child(fire_new)
		fire_new.global_position = plant_cell.global_position + Vector2(plant_cell.size.x, plant_cell.size.y / 2)
		fire_new.activate_bomb_effect()
...
func jalapeno_bomb_item_lane_row(lane:int):
	## 梯子
	for p_c :PlantCell in all_plant_cells[lane]:
		if is_instance_valid(p_c.ladder):
			p_c.ladder.queue_free()
