...
var all_pages_array : GridContainer
...
func _ready() -> void:
	...
	var page = $Scrooll/GridContainer
	all_pages_array=page
	page.visible = false
	for node in page.get_children():
		...
		if node is ChooseLevelButton:
			...
			## 初始化游戏数据的选关数据
			node.curr_level_data_game_para.set_choose_level(game_mode, 1, level_id)
...
func _ready_update_page():
	...
	$Scrooll/GridContainer.visible = true
